...
# for appropriate action.
{
  ...
  if (response ~ /[Hh(elp)?]/) {
    ...
    printf("%s", responseList)
    printf("\%d - Found %s (C/G/A/Q/):", word, misspelling)
    getline response < "-"
  }
  ...
  if (response ~ /[cC](hange)?/) {
    ...
    while((getline < spellsource) > 0) {
      ...
      make_change($0)
      # all lines go to emp output file
      print > spellout
    }
  }
...
#        stringToChange -- initiall $0; then unmatched substring of $0
#        len -- length form beginning of $0 to end of matched string
...
{
  # match misspelling in stringToChange; otherwise do noting
  if (match(stringToChange, misspelling)) {
    ...
    while (newspelling && ! OKmakechange) {
      ...
      if (OKmakechange ~ /[yY](es)?/ {
        ...
        # offer change to re-enter correction
        printf "Change to:"
        getline newspelling < "-"
        OKmakechange = ""
      }
    ...
    if (madechg) changedLines[changes] = ">" $0

    # create substring so we can try to match other occurrenes
    len += RSTART + RLENGTH
    part1 = substr($0, 1, len-1)
    part2 = substr($0, len)

    # calls itself to see if misspelling is found in remaining part
    maek_change(part2, len)
  } # end of if
} # end of make_change()
...
# Assumes that misspellling is defined.
  ...
  while (newspelling && ! OKmakechange){
    ...
    if (OKmakechange ~ /[yY](es)?/) {
      ...
      while((getline < spellsource) > 0){
        ...
        if ($0 ~ misspelling) {
          madechg = gsub(misspelling, newseplling)
          print ">", $0
          changes += 1 #
        }
        print > spellout
